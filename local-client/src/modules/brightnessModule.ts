...
export class BrightnessModule {
  ...
  private mapAction(action: any): string {
    switch (action.action) {
      case 'increase': return 'sudo brightnessctl set +10%';
      case 'decrease': return 'sudo brightnessctl set 10%-';
      case 'set':
        if (action.value !== undefined) {
          return `sudo brightnessctl set ${action.value}%`;
        }
        throw new Error('Set brightness requires value');
      default: throw new Error(`Unknown brightness action: ${action.action}`);
    }
  }
}